# Contributing to ConoHa Skills

ConoHa Skills へのコントリビュートに興味を持っていただきありがとうございます。

## 📝 コントリビューション方針

**現在の受け入れ体制**: このプロジェクトは現在、Issue（バグ報告・機能要望・質問）のみを受け入れています。

**将来の計画**: プルリクエストの受け入れは現在検討中です。
更新があればお知らせします。
...
2. **適切なテンプレート選択**: バグ報告、機能要望に応じたテンプレートを使用
...
1. `plugins/*/.claude-plugin/plugin.json` の `version` を適切に更新（必要に応じて `.claude-plugin/marketplace.json` のエントリも更新）
